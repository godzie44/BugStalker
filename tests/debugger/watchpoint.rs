...
use bugstalker::debugger::variable::select::{DQE, VariableSelector};
...
fn test_watchpoint_works() {
    ...
    let wp_dqe = DQE::Variable(VariableSelector::by_name("int8", true));
    debugger
...
fn test_max_watchpoint_count() {
    ...
    let wp_dqe = DQE::Variable(VariableSelector::by_name("a", false));
    dbg.set_watchpoint_on_expr("a", wp_dqe, DataWrites).unwrap();
    let wp_dqe = DQE::Variable(VariableSelector::by_name("b", false));
    dbg.set_watchpoint_on_expr("b", wp_dqe, DataWrites).unwrap();
    let wp_dqe = DQE::Variable(VariableSelector::by_name("c", false));
    dbg.set_watchpoint_on_expr("c", wp_dqe, DataWrites).unwrap();
    let wp_dqe = DQE::Variable(VariableSelector::by_name("d", false));
...
fn test_watchpoint_global_var_multithread() {
    ...
    let wp_dqe = DQE::Field(
        DQE::Field(
            DQE::Variable(VariableSelector::by_name("GLOBAL_2", false)).boxed(),
            "data".to_string(),
        )
        .boxed(),
        "value".to_string(),
