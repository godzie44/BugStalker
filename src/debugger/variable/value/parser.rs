...
use crate::debugger::variable::value::{
    ...
    ScalarValue, StructValue, SubroutineValue, SupportedScalar, Value,
};
...
impl ValueModifiers {
    pub fn from_identity(ident: Identity) -> ValueModifiers {
...
impl ValueParser {
                    ...
                    raw_items_iter
                        .map(|(i, chunk)| {
                            ...
                            ArrayItem {
                                ...
                                value: self.parse_inner(ctx, Some(data), el_type_id),
                            }
                        })
                        .collect::<Vec<_>>(),
                )
    ...
    fn parse_c_enum(
        ...
        let in_debugee_loc = data.as_ref().and_then(|d| d.address);
        let mb_discr = discr_type.map(|type_id| self.parse_inner(ctx, data, type_id));
    ...
    fn parse_inner_with_modifiers(
        ...
        match &type_graph.types[&type_id] {
            TypeDeclaration::Scalar(scalar_type) => {
                Value::Scalar(self.parse_scalar(data, type_id, scalar_type))
            }
            TypeDeclaration::Structure {
                ...
                Value::Struct(struct_var)
            }
            TypeDeclaration::Array(decl) => {
                Value::Array(self.parse_array(ctx, data, type_id, decl))
            }
            TypeDeclaration::CStyleEnum {
                discr_type,
                enumerators,
                ..
            } => Value::CEnum(self.parse_c_enum(ctx, data, type_id, *discr_type, enumerators)),
            TypeDeclaration::RustEnum {
                discr_type,
                enumerators,
                ..
            } => Value::RustEnum(self.parse_rust_enum(
                ctx,
                data,
                type_id,
            ...
            TypeDeclaration::Pointer { target_type, .. } => {
                Value::Pointer(self.parse_pointer(ctx, data, type_id, *target_type))
            }
            ...
            TypeDeclaration::Subroutine { return_type, .. } => {
                ...
                Value::Subroutine(fn_var)
            }
            TypeDeclaration::ModifiedType {
                inner, modifier, ..
            } => {
                let in_debugee_loc = data.as_ref().and_then(|d| d.address);
                Value::CModifiedVariable(CModifiedValue {
                    type_id: Some(type_id),
                    type_ident: ctx.type_graph.identity(type_id),
                    modifier: *modifier,
                    value: inner
                        .map(|inner_type| Box::new(self.parse_inner(ctx, data, inner_type))),
